...
static int cat_sockaddr__getbyname(cat_sockaddr_t *address, cat_socklen_t *address_length, const char *name, size_t name_length, int port)
{
    ...
    if (af == AF_LOCAL) {
        ...
        *address_length = offsetof(cat_sockaddr_local_t, sl_path) + name_length;
...
#define CAT_SOCKET_IS_ESTABLISHED(_isocket) ((_isocket)->connected) /* TODO: SSL handshake */

#define CAT_SOCKET_ESTABLISHED_ONLY(_isocket, _failure) do { \
    if (unlikely(!CAT_SOCKET_IS_ESTABLISHED(_isocket))) { \
...
#define CAT_SOCKET_SERVER_ONLY(_socket, _failure) \
    CAT_SOCKET_WHICH_ONLY(_socket, CAT_SOCKET_TYPE_FLAG_SERVER, "Socket is not listening for connections", _failure)

#define CAT_SOCKET_ESTABLISHED_ONCE(_isocket, _member, _failure) do { \
    if (_isocket->_member) { \
        cat_update_last_error(CAT_EISCONN, "Socket has been " #_member); \
        _failure; \
    } \
} while (0)
...
CAT_API cat_bool_t cat_socket_connect_ex(cat_socket_t *socket, const char *name, size_t name_length, int port, cat_timeout_t timeout)
{
    ...
    CAT_SOCKET_ESTABLISHED_ONCE(isocket, connected, return cat_false);
    cat_sockaddr_info_t address_info;
    cat_sockaddr_t *address;
    cat_socklen_t address_length;
...
#ifdef CAT_SSL
CAT_API cat_bool_t cat_socket_ssl_handshake(cat_socket_t *socket)
{
    CAT_SOCKET_INTERNAL_GETTER_WITH_IO(socket, isocket, CAT_SOCKET_IO_FLAG_HANDSHAKE, return -1);
    CAT_SOCKET_ESTABLISHED_ONLY(isocket, return -1);
    // CAT_SOCKET_ESTABLISHED_ONCE(isocket, handshaked, return cat_false);

    // TODO
    socket->type |= CAT_SOCKET_TYPE_FLAG_SSL;
...
static void cat_socket_read_alloc_callback(uv_handle_t *handle, size_t suggested_size, uv_buf_t *buf)
{
    ...
    buf->len = context->size - context->nread;
}
...
static ssize_t cat_socket_internal_read(
...
static cat_never_inline cat_bool_t cat_socket_internal_udg_write(
    cat_socket_internal_t *isocket,
    const cat_socket_write_vector_t *vectors, unsigned int vector_count,
    const cat_sockaddr_t *address, cat_socklen_t address_length,
    cat_timeout_t timeout
)
{
...
static cat_bool_t cat_socket_internal_write(
    cat_socket_internal_t *isocket,
    const cat_socket_write_vector_t *vectors, unsigned int vector_count,
    const cat_sockaddr_t *address, cat_socklen_t address_length,
    cat_timeout_t timeout
)
{
...
#ifdef CAT_OS_UNIX_LIKE
    if (is_dgram && !is_udp) {
        return cat_socket_internal_udg_write(isocket, vectors, vector_count, address, address_length, timeout);
    }
#endif
    ...
    if (!is_dgram) {
        error = uv_write(
            ...
            (const uv_buf_t *) vectors, vector_count,
            cat_socket_write_callback
        );
    } else {
        error = uv_udp_send(
            &request->u.udp, &isocket->u.udp,
            (const uv_buf_t *) vectors, vector_count, address,
...
#define CAT_SOCKET_IO_CHECK(_socket, _isocket, _io_flag) \
    ...
    do { \
        if ((_socket->type & CAT_SOCKET_TYPE_FLAG_DGRAM) != CAT_SOCKET_TYPE_FLAG_DGRAM) { \
            CAT_SOCKET_ESTABLISHED_ONLY(_isocket, return -1); \
        } \
    } while (0)
...
static cat_always_inline cat_bool_t cat_socket__write(cat_socket_t *socket, const cat_socket_write_vector_t *vectors, unsigned int vector_count, const cat_sockaddr_t *address, cat_socklen_t address_length, cat_timeout_t timeout)
{
    CAT_SOCKET_IO_CHECK(socket, isocket, CAT_SOCKET_IO_FLAG_NONE);
    return cat_socket_internal_write(isocket, vectors, vector_count, address, address_length, timeout);
}
...
static cat_bool_t cat_socket__write_to(cat_socket_t *socket, const cat_socket_write_vector_t *vectors, unsigned int vector_count, const char *name, size_t name_length, int port, cat_timeout_t timeout)
{
    CAT_SOCKET_IO_CHECK(socket, isocket, CAT_SOCKET_IO_FLAG_NONE);
    cat_sockaddr_info_t address_info;
    cat_sockaddr_t *address;
...
{
    return cat_socket__write(socket, vectors, vector_count, NULL, 0, timeout);
}
...
{
    return cat_socket__write(socket, vectors, vector_count, address, address_length, timeout);
}
...
{
    return cat_socket__write_to(socket, vectors, vector_count, name, name_length, port, timeout);
}
...
)
{
    ...
    do {
#endif
        nread = recvfrom(fd, buffer, size, MSG_PEEK, address, address_length);
#ifdef CAT_OS_UNIX_LIKE
...
#ifdef CAT_SSL
    } else if (isocket->io_flags == CAT_SOCKET_IO_FLAG_HANDSHAKE) {
        cat_socket_cancel_io(isocket->context.handshake.coroutine, "handshake");
#endif
    } else {
...
{
    cat_socket_internal_t *isocket = socket->internal;
    return isocket != NULL && CAT_SOCKET_IS_ESTABLISHED(isocket);
}
...
CAT_API const char *cat_socket_io_state_name(cat_socket_io_flags_t io_state)
{
    switch (io_state) {
        ...
        case CAT_SOCKET_IO_FLAG_CONNECT:
            return "connect";
#ifdef CAT_SSL
        case CAT_SOCKET_IO_FLAG_HANDSHAKE:
            return "handshake";
#endif
